...
conn <- dbConnect(RSQLite::SQLite(), 'recomendations.sqlite')

# read initial data in database under country_data
...
# create vertical tags from country_data
...
# create edges for videos, tags for each country
...
recommendations('peter', '3HhK7HbX3Qs', 'US', 20)

recommendations('ronald', 'zjgZuuE8OVc', 'US', 40) %>%
  print(n = 40)

dbExecute(conn, 'drop table peter_tags_preferences')


4*((N-p)/N)*(1-((N-p)/N))*weight + (weight*(1-(p/C)))



like_videos(c('black', 'comedy', 'cooking'), 20, 'US')
